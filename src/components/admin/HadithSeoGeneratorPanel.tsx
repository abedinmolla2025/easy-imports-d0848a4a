...
export default function HadithSeoGeneratorPanel() {
  ...
  const start = useCallback(async () => {
    ...
    toast({
      ...
      description: `${totalSoFar} টি হাদিসে SEO content যোগ হয়েছে। ${failSoFar} টি ব্যর্থ।`,
    });
    loadStats();
  ...
  const sessionTotal = processedTotal + failedTotal;
  const sessionTarget = Math.min(pending + sessionTotal, pending + sessionTotal); // running denom
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          প্রতিটি হাদিসের জন্য AI দিয়ে বাংলা <code>explanation_bn</code> ও <code>lessons_bn</code> তৈরি করো।
      ...
      <CardContent className="space-y-4">
        <div className="grid grid-cols-3 gap-3">
          <div className="rounded-lg border p-3 text-center">
            ...
            <p className="text-xs text-muted-foreground">মোট হাদিস</p>
          </div>
          <div className="rounded-lg border p-3 text-center">
